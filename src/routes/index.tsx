...
const categoryConfig: Record<string, { color: string; soft: string; Icon: React.ElementType }> = {
  scholarships:      { color: "#60a5fa", soft: "rgba(59, 130, 246, 0.12)",  Icon: GraduationCap },
  volunteering:      { color: "#4ade80", soft: "rgba(34, 197, 94, 0.12)",   Icon: HandHeart },
  competitions:      { color: "#fbbf24", soft: "rgba(234, 179, 8, 0.12)",   Icon: Trophy },
  internships:       { color: "#a78bfa", soft: "rgba(139, 92, 246, 0.14)",  Icon: Briefcase },
  "summer-programs": { color: "#fb923c", soft: "rgba(249, 115, 22, 0.12)",  Icon: Sun },
  grants:            { color: "#f472b6", soft: "rgba(236, 72, 153, 0.12)",  Icon: Banknote },
};
...
function OppCard({ o }: { o: Opportunity }) {
  ...
  return (
    <article className="opp-card">
      {cfg && (
        <div className="opp-banner" style={{ background: cfg.soft }}>
          <cfg.Icon size={34} strokeWidth={1.5} color={cfg.color} />
        </div>
      )}
      <div className="opp-top">
        <span className="cat-badge" style={cfg ? { background: cfg.soft, color: cfg.color } : undefined}>
          {o.category.replace("-", " ")}
        </span>
