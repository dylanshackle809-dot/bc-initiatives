...
const categoryConfig: Record<string, { bg: string; color: string; Icon: React.ElementType; label: string }> = {
  scholarships:      { bg: "rgba(59, 130, 246, 0.10)",  color: "#60a5fa", Icon: GraduationCap, label: "Scholarship" },
  volunteering:      { bg: "rgba(34, 197, 94, 0.10)",   color: "#4ade80", Icon: HandHeart, label: "Volunteering" },
  competitions:      { bg: "rgba(234, 179, 8, 0.10)",   color: "#fbbf24", Icon: Trophy, label: "Competition" },
  internships:       { bg: "rgba(139, 92, 246, 0.12)",  color: "#a78bfa", Icon: Briefcase, label: "Internship" },
  "summer-programs": { bg: "rgba(249, 115, 22, 0.10)",  color: "#fb923c", Icon: Sun, label: "Summer Program" },
  grants:            { bg: "rgba(236, 72, 153, 0.10)",  color: "#f472b6", Icon: Banknote, label: "Grant" },
};
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "1rem", flexWrap: "wrap" }}>
            ...
            {opp.stem && (
              <span style={{ background: "rgba(2, 132, 199, 0.15)", color: "#38bdf8", borderRadius: 999, padding: "3px 12px", fontSize: "0.7rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>STEM</span>
            )}
            {opp.international && (
              <span style={{ background: "rgba(139, 92, 246, 0.15)", color: "#a78bfa", borderRadius: 999, padding: "3px 12px", fontSize: "0.7rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", display: "inline-flex", alignItems: "center", gap: "0.3rem" }}>
                <Globe size={10} strokeWidth={2} />International
              </span>
            )}
            <span style={{
              background: opp.deadlineStatus === "open" ? "rgba(34, 197, 94, 0.15)" : "rgba(234, 179, 8, 0.15)",
              color: opp.deadlineStatus === "open" ? "#4ade80" : "#fbbf24",
              borderRadius: 999, padding: "3px 12px",
              fontSize: "0.7rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em"
            }}>
        ...
        <div className="detail-aside">
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: 12, padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            <a href={opp.link} target="_blank" rel="noreferrer"
              style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", background: "var(--accent)", color: "#FFFFFF", borderRadius: 10, fontWeight: 600, fontSize: "0.95rem", padding: "0.85rem 1.5rem", textDecoration: "none", boxShadow: "0 0 20px rgba(139, 92, 246, 0.35)", transition: "box-shadow 200ms, background 150ms" }}
          ...
          <div style={{ background: cfg?.bg ?? "var(--bg-secondary)", border: `1px solid ${cfg?.color ?? "var(--border-color)"}33`, borderRadius: 12, padding: "1.25rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.82rem" }}>
                ...
                <span style={{ fontWeight: 600, color: opp.compensated ? "#4ade80" : "var(--text-secondary)" }}>{opp.compensated ? "Yes" : "No"}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.82rem" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.82rem" }}>
                ...
                <span style={{ fontWeight: 600, color: opp.difficulty === "Competitive" ? "#f87171" : opp.difficulty === "Moderate" ? "#fbbf24" : "#4ade80" }}>{opp.difficulty}</span>
              </div>
              {opp.stem && (
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.82rem" }}>
                  <span style={{ color: "var(--text-secondary)" }}>STEM focus</span>
                  <span style={{ fontWeight: 600, color: "#38bdf8" }}>Yes</span>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="related-grid">
                {related.map((r) => (
                  <Link key={r.id} to="/opportunities/$id" params={{ id: String(r.id) }}
                    ...
                    onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 0 32px rgba(124, 58, 237, 0.16)"; e.currentTarget.style.borderColor = "rgba(139, 92, 246, 0.45)"; e.currentTarget.style.transform = "translateY(-3px)"; }}
